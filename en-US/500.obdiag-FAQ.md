...
A: In OceanBase Diagnostic Tool (obdiag) of a version earlier than V3.2.0, collected information is compressed by using a zip tool on the target host. Therefore, a zip tool must be installed on the target host whose information is to be collected.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This issue has been resolved since obdiag V3.2.0. </p>
</main>
...
A: Enable password-free SSH connection between the remote node, which is generally an OBServer node, and the node where obdiag is deployed. For example, to implement password-free connection from server A to server B, perform the following steps:
