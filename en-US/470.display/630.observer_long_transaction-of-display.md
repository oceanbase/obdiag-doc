...
This article is applicable to the scenario of independent deployment of obdiag. It can be executed with one click to display the long transaction information of the cluster.
...
| --env | Yes | string | Default is empty | Additional parameters required by obdiag display, here used to set the time threshold for transaction execution, in seconds. Fixed style: `--env wait_time=${time}`. |
