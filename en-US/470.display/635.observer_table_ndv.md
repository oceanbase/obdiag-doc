...
This article is applicable to the scenario of independent deployment of obdiag, which can query table ndv information with one click.
...
| --env | Yes | string | Default is empty | Additional parameters required by obdiag display, used here to configure the information of the table to be queried. Fixed style: `--env database_name=${db_name} --env table_name=${table_name}`.     |
