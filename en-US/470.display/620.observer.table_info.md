...
This article is applicable to the scenario of independent deployment of obdiag. It can be executed with one click to display the table information of the cluster in the terminal.
...
| --env | Yes | string | Default is empty | Additional parameters required by obdiag display, support specifying key=value format multiple times, for example: `--env host=127.0.0.1 --env port=2881 --env user=test@test --env password=****** --env database=test --env database_name=test --env table_name=test`.     |
