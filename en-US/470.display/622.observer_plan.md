...
This article is applicable to the scenario of independent deployment of obdiag. It can be executed with one click to display the SQL execution plan information on the terminal.
...
| --env | Yes | string | Default is empty | Additional parameters required for obdiag display, fixed style: `--env tenant_name=test --env sqlid=test --env sqlid=test`. |
