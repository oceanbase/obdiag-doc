...
If OceanBase Diagnostic Tool (obdiag) is independently deployed, you can use the `obdiag check` commands to inspect the status of an OceanBase cluster. The inspection process analyzes the cluster from multiple perspectives such as the system kernel parameters and internal tables, identifies the causes of existing or possible exceptions, and provides O&M suggestions.
