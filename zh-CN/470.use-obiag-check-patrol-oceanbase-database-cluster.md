# 通过 obdiag check 巡检 OceanBase 数据库集群
...
|-------------|------|--------|---------------------------|--------------------------------------|
| --cases     | 否    | string | `` | 需要执行的巡检项目的集合名。当未赋值时执行所有的 tasks。 |
...
```shell
#先进入${CHECK.tasks_base_path}，然后创建一个文件夹 test，并创建示例文件 test.yaml
cd ~/.obdiag/tasks/
mkdir test
cd test
touch test.yaml
...
| obversion | 否 | 表示适用的版本，用 Str 的形式表示范围，需要完整的数字的版本号。<ul><li>OceanBase 数据库 V3.x 版本为三位，例如 [3.1.1,3.2.0]。</li><li> OceanBase 数据库 V4.x 版本为四位，例如：[4.1.0.0,4.2.0.0]。</li></ul> |
...
| result    | 否    | 结构为一个单独的对象，用于对这个步骤结束后需要进行的操作进行解析，如校验结果逻辑，逻辑不通过时需要报错的文本信息进行说明等等。具体可参考 [result & verify 功能](#result%20&%20verify%20功能) |
...
| err_msg     | 否    | 用于非正常执行时答应的日志，支持配置全局变，在 `verify` 为 `false` 的时候所输出的 msg 建议配置了 `verify`，就一定要配上 `err_msg`。         |
...
* `equal`：判断 `set_value` 的值是否等于 `verify` 提供的值，需要注意的是 `verify_type` 兼容字符串或 int，`${set_value}` 和 `verify` 必须是同类型 。
...
为了更好地提供更有效的检测项任务，我们会不定时更新 task 的数量和质量，用户可以在 [obdiag 的代码仓库](https://github.com/oceanbase/oceanbase-diagnostic-tool) 中复制 `${code_path}/handler/check/tasks` 文件夹来替换原有文件夹 `~/.obdiag/tasks`，同时需要的还有 check_package.yaml(`~/.obdiag/check_package.yaml`)。
