...
| --since  | 否    | string | `30m`        | 日志收集最近的某段时间，格式为: `<n><m|h|d>`，如 `30m` 表示收集最近 30 分钟的日志。  |
| --env | 否    | string | 默认为空                | 部分场景需要额外的参数才能支持，可通过 `--env` 设置，格式：`--env key=value`，可多次指定。例如：`--env host=127.0.0.1 --env port=2881 --env user=test@test --env password=****** --env database=test`。具体配置方式可参见对应场景介绍文档。  |
| --store_dir | 否 | string | 默认为命令执行的当前路径 | 存储结果的本地路径。 |
| --temp_dir | 否 | string | `/tmp` | 远端节点在采集过程中产生的临时文件存储目录。 |
| --skip_type | 否 | string | 默认为空 | 要跳过的采集类型，可配置值为 `ssh`、`sql`。多个值间使用英文逗号（`,`）分隔。 |
| --redact | 否 | string | 默认为空 | 选择 OceanBase 集群日志脱敏规则，例如可配置为 `all_sql`，使用多个规则时使用英文逗号（`,`）分隔，编写规则参考本文 [脱敏规则编写](../100.normal/101.gather_log.md) 介绍。 |
