...
| --skip_type | 否 | string | 默认为空 | 要跳过的采集类型，可选：`ssh`、`sql`。多个用英文逗号分隔。 |
| --redact | 否 | string | 默认为空 | observer 日志脱敏规则，如 `all_sql`；多个规则用英文逗号分隔。详见 [脱敏规则编写](../100.normal/101.gather_log.md)。 |
