...
| --redact       | 否    | string | 默认为空                    | 选择脱敏规则，例如可配置为 `all_sql`，使用多个规则时使用英文逗号（`,`）分隔，编写规则参考本文 [脱敏规则编写](#脱敏规则编写) 介绍。<main id="notice" type='explain'><h4>说明</h4><li> 脱敏结果的 zip 文件不进行加密处理，若需加密请手动进行。</li></ul></main>     |
