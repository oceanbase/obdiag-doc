...
本文将介绍 `obdiag tool crypto_config` 命令的各个选项及命令使用。
...
|-------------|------|--------|----------|------------------------------------------------------------------------------------------------------|
| --config_password     | 否    | string | 默认为空     | 使用obdiag 加密配置文件时传入的密码      |
| --key | 否    | string | 默认为 `./rca` | 加密/解密配置文件所需的密码              |
| --file | 否    | string | 默认为空 | 需要加密/解密的配置文件路径                     |
| --encrypted_file | 否    | string | 默认为空 | 密码验证的时候需要的加密配置文件路径                    |
