...
obdiag 自 V4.0.0 起支持通过 `obdiag tool config_check` 命令校验配置文件的连接状态，本文将介绍具体使用。
...
| 错误信息 | 可能原因 | 解决方案 |
|---------|---------|---------|
| Connection refused | 数据库服务未启动或端口错误 | 检查 OBServer 是否正常运行，确认端口配置正确 |
...
1. 建议在执行其他 obdiag 命令前先运行配置校验，确保连接正常。
2. 该命令会尝试连接配置文件中的所有节点，可能需要一定时间。
3. 如果使用 SSH 密钥认证，请确保密钥文件路径正确且权限设置正确。
