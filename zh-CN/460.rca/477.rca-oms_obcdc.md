...
|slug|oms-obcdc-component-analysis-oms_obcdc|

# 根因分析场景：OMS obcdc 组件分析 oms_obcdc
...
|--------------|------|--------|-----|----------------------------------------|
| component_id | 是    | string | ""  | 组件 ID，可访问 OMS 控制台 -> **运维监控** -> **组件** -> {目标组件类型} -> **名称** 获取，名称即为对应的组件 ID。一般组成为 `{IP 信息}_xxxxx` |
