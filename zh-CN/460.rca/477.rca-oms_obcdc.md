# 根因分析场景：OMS obcdc组件分析 oms_obcdc
...
|--------------|------|--------|-----|----------------------------------------|
| component_id | 是    | string | ""  | 任务 ID。全量迁移的任务 ID，一般组成为 `{IP 信息}_xxxxx` |
...
component_id即为组件id。 可从 控制台 -> 运维监控 -> 组件 -> {目标组件类型} -> 名称 中获取， 名称即为对应的组件id。

## obdiag 集群配置信息

使用 oms 相关诊断时（ scene 名以 oms 开头）需要配置 oms 集群信息， 示例如下：

默认为 `~/.obdiag/config.yml`， 也可以通过 `--config` 选项指定配置文件。
