...
OceanBase 数据库是原生分布式数据库系统，故障根因分析通常是比较繁琐的，因为涉及的因素可能有很多，如机器环境、配置参数、运行负载等等。专家在排查问题的时候需要获取大量的信息来分析故障，如何高效的采集和分析故障场景下分散在各个节点的信息，便是 OceanBase 敏捷诊断工具(OceanBase Diagnostic Tool) 简称 obdiag，需要解决的问题。obdiag代码完全开源，[github代码仓库](https://github.com/oceanbase/obdiag)
