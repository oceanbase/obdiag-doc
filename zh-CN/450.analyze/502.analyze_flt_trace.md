...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>自 V4.1.0 起，obdiag analyze flt_trace 支持按客户端 IP 自动过滤 OBProxy 节点。分析过程中会先从 SQL 审计视图（<code>gv$ob_sql_audit</code> 或 <code>gv$sql_audit</code>）获取与 flt_trace_id 相关的客户端 IP，然后仅从匹配的 OBProxy 节点收集日志，提升分析效率并减少不必要的数据收集。该功能兼容多版本 OceanBase 数据库。</p>
</main>
