...
| --scope | 否 | string | all | 要分析的日志类型：`observer`、`election`、`rootservice`、`all`。 |
| --grep | 否 | string | 默认为空 | 关键字过滤，可多次指定。 |
| --log_level | 否 | string | 默认为空 | 仅分析不低于该等级的日志。可选：`DEBUG`、`TRACE`、`INFO`、`WDIAG`、`WARN`、`EDIAG`、`ERROR`。 |
| --files | 否 | string | 默认为空 | 离线分析模式：指定本地日志文件或目录；指定后忽略 `--from`、`--to`、`--since`，从本地文件分析。 |
| --store_dir | 否 | string | 当前目录 | 分析结果存储路径。 |
| --temp_dir | 否 | string | `/tmp` | 在线分析时远端节点临时文件目录。 |
