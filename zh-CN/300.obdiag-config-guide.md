...
执行完成后在 `~/.obdiag/config.yml` 中会生成一份新的配置，如果原来`~/.obdiag/config.yml` 存在内容，将会将老配置备份到~/.obdiag/backup_conf目录下。
...
obcluster:
  ...
  servers:
    nodes:
      ...
      - ip: xx.xx.xx.3
    global:
      ssh_username: admin # 登录信息，建议使用部署observer时所使用的用户信息
      ssh_password: admin # 若不使用密码登录或无密码，可设置为“”
...
obproxy:
  ...
  servers:
    nodes:
      ...
      - ip: xx.xx.xx.6
    global:
      ssh_username: admin # 登录信息，建议使用部署obproxy时所使用的用户信息
      ssh_password: admin # 若不使用密码登录或无密码，可设置为“”
      # ssh_port: 22 # ssh端口，默认为22
      # ssh_key_file: "" # ssh秘钥地址，与ssh_password二选一即可
