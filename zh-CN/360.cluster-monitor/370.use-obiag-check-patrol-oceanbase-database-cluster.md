...
| --observer_tasks | 否 | string | 默认为空 | 指定要执行的 Observer 巡检 task，多个用英文分号（`;`）分隔。指定后忽略 `--cases` 中的 observer 部分。 |
| --obproxy_tasks | 否 | string | 默认为空 | 指定要执行的 OBProxy 巡检 task，多个用英文分号（`;`）分隔。指定后忽略 `--obproxy_cases`。 |
| --store_dir | 否 | string | `./check_report/` | 巡检报告输出目录。 |
| --report_type | 否 | string | table | 巡检报告输出格式：table、json、xml、yaml、html。 |
...
为了更好地提供更有效的检测项任务，我们会不定时更新 task 的数量和质量。用户可从 [obdiag 代码仓库](https://github.com/oceanbase/obdiag) 的 `plugins/check/` 目录获取最新巡检任务与套餐，复制到本地 `~/.obdiag/check/tasks`（或系统配置 `check.tasks_base_path` 所指定目录），并同步更新 `*check_package.yaml`（如 `~/.obdiag/check/*check_package.yaml`）以更新巡检套餐。也可通过 `obdiag update` 进行热更新。
